...
def get_request(path, settings=None, params={}):
...
def get_filtering_condition():
    ...
    if woocommerce_settings.last_sync_datetime:
        ...
        return { "modified_after": last_sync_datetime.isoformat() }
    return {}
...
def get_woocommerce_items(ignore_filter_conditions=False):
    ...
    if not ignore_filter_conditions:
        ...
        if cint(frappe.get_value("WooCommerce Config", "WooCommerce Config", "sync_only_published")) == 1:
            filter_condition.update({"status":"publish"})

    response = get_request_request('products', params={"per_page":_per_page,**filter_condition})
    woocommerce_products.extend(response.json())

    for page_idx in range(1, int( response.headers.get('X-WP-TotalPages')) or 1):
        response = get_request_request('products', params={"per_page":_per_page, "page": page_idx+1,**filter_condition})
        woocommerce_products.extend(response.json())
...
def get_woocommerce_item_variants(woocommerce_product_id):
    ...
    response = get_request_request('products/{0}/variations'.format(woocommerce_product_id), params={"per_page":_per_page, **filter_condition})
    woocommerce_product_variants.extend(response.json()) 
    
    for page_idx in range(1, int( response.headers.get('X-WP-TotalPages')) or 1):
        response = get_request_request('products/{0}/variations'.format(woocommerce_product_id), params={"per_page":_per_page, "page": page_idx+1,**filter_condition})
        woocommerce_product_variants.extend(response.json())
...
def get_woocommerce_orders(order_status):
    ...
    response = get_request_request('orders',params={"per_page":_per_page,"status":order_status})
    woocommerce_orders.extend(response.json())
        
    for page_idx in range(1, int( response.headers.get('X-WP-TotalPages')) or 1):
        response = get_request_request('orders',params={"per_page":_per_page,"page":page_idx+1,"status":order_status})
        woocommerce_orders.extend(response.json())
...
def get_woocommerce_customers(ignore_filter_conditions=False):
    ...
    if not ignore_filter_conditions:
        ...
        response = get_request_request('customers', params={"per_page":_per_page,**filter_condition})
        woocommerce_customers.extend(response.json())

        for page_idx in range(1, int( response.headers.get('X-WP-TotalPages')) or 1):
            response = get_request_request('customers', params={"per_page":_per_page, "page": page_idx+1,**filter_condition})
            woocommerce_customers.extend(response.json())
