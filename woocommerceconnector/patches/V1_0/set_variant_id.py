...
def execute():
	...
	if woocommerce_settings.woocommerce_url and woocommerce_items:
		for item in frappe.db.sql("""select name, item_code, woocommerce_product_id, has_variants, variant_of from tabItem
			where sync_with_woocommerce=1 and woocommerce_product_id is not null""", as_dict=1):

			if item.get("variant_of"):
				frappe.db.sql(""" update tabItem set woocommerce_variant_id=woocommerce_product_id
					where name = %s """, item.get("name"))

			elif not item.get("has_variants"):
				product = filter(lambda woocommerce_item: woocommerce_item['id'] == cint(item.get("woocommerce_product_id")), woocommerce_items)
